...
def _printconstant_verilog(node):
    if node.signed:
...
def _printexpr_verilog(ns, node):
    if isinstance(node, Constant):
        return _printconstant_verilog(node)
    elif isinstance(node, Signal):
        return ns.get_name(node), node.signed
    elif isinstance(node, _Operator):
        arity = len(node.operands)
        r1, s1 = _printexpr_verilog(ns, node.operands[0])
        if arity == 1:
            if node.op == "-":
                if s1:
                    r = node.op + r1
            ...
            r2, s2 = _printexpr_verilog(ns, node.operands[1])
            if node.op not in ["<<<", ">>>"]:
                if s2 and not s1:
                    r1 = "$signed({1'd0, " + r1 + "})"
                if s1 and not s2:
            ...
            assert node.op == "m"
            r2, s2 = _printexpr_verilog(ns, node.operands[1])
            r3, s3 = _printexpr_verilog(ns, node.operands[2])
            if s2 and not s3:
                r3 = "$signed({1'd0, " + r3 + "})"
            if s3 and not s2:
                r2 = "$signed({1'd0, " + r2 + "})"
        ...
        if isinstance(node.value, Signal) \
          ...
          and node.start == 0 and node.stop == 1:
              return _printexpr_verilog(ns, node.value)

        if node.start + 1 == node.stop:
            sr = "[" + str(node.start) + "]"
        else:
            sr = "[" + str(node.stop-1) + ":" + str(node.start) + "]"
        r, s = _printexpr_verilog(ns, node.value)
        return r + sr, s
    elif isinstance(node, Cat):
        l = [_printexpr_verilog(ns, v)[0] for v in reversed(node.l)]
        return "{" + ", ".join(l) + "}", False
    elif isinstance(node, Replicate):
        return "{" + str(node.n) + "{" + _printexpr_verilog(ns, node.v)[0] + "}}", False
    else:
...
def _printnode_verilog(ns, at, level, node):
    if isinstance(node, _Assign):
        if at == _AT_BLOCKING:
            ...
            assignment = " <= "
        return "\t"*level + _printexpr_verilog(ns, node.l)[0] + assignment + _printexpr_verilog(ns, node.r)[0] + ";\n"
    elif isinstance(node, collections.Iterable):
        return "".join(list(map(partial(_printnode_verilog, ns, at, level), node)))
    elif isinstance(node, If):
        r = "\t"*level + "if (" + _printexpr_verilog(ns, node.cond)[0] + ") begin\n"
        r += _printnode_verilog(ns, at, level + 1, node.t)
        if node.f:
            r += "\t"*level + "end else begin\n"
            r += _printnode_verilog(ns, at, level + 1, node.f)
        r += "\t"*level + "end\n"
        return r
    elif isinstance(node, Case):
        if node.cases:
            r = "\t"*level + "case (" + _printexpr_verilog(ns, node.test)[0] + ")\n"
            css = [(k, v) for k, v in node.cases.items() if isinstance(k, Constant)]
            css = sorted(css, key=lambda x: x[0].value)
            for choice, statements in css:
                r += "\t"*(level + 1) + _printexpr_verilog(ns, choice)[0] + ": begin\n"
                r += _printnode_verilog(ns, at, level + 2, statements)
                r += "\t"*(level + 1) + "end\n"
            if "default" in node.cases:
                r += "\t"*(level + 1) + "default: begin\n"
                r += _printnode_verilog(ns, at, level + 2, node.cases["default"])
                r += "\t"*(level + 1) + "end\n"
            r += "\t"*level + "endcase\n"
            return r
        else:
...
def _printentitydecl(f, ios, name, ns,
                         reg_initialization):
...
entity {name} is
    port(
        """.format(name)
    r += ';\n        '.join(
        _printsig(ns, sig, 'inout' if sig in inouts else 'out' if sig in targets else 'in')
...
def _printheader_verilog(f, ios, name, ns,
                         reg_initialization):
    sigs = list_signals(f) | list_special_ios(f, True, True, True)
    special_outs = list_special_ios(f, False, True, True)
    inouts = list_special_ios(f, False, False, True)
    ...
    for sig in sorted(ios, key=lambda x: x.duid):
        ...
        firstp = False
        if sig in inouts:
            r += "\tinout " + _printsig_verilog(ns, sig)
        elif sig in targets:
            if sig in wires:
                r += "\toutput " + _printsig_verilog(ns, sig)
            else:
                r += "\toutput reg " + _printsig_verilog(ns, sig)
        else:
            r += "\tinput " + _printsig_verilog(ns, sig)
    r += "\n);\n\n"
    for sig in sorted(sigs - ios, key=lambda x: x.duid):
        if sig in wires:
            r += "wire " + _printsig_verilog(ns, sig) + ";\n"
        else:
            if reg_initialization:
                r += "reg " + _printsig_verilog(ns, sig) + " = " + _printexpr_verilog(ns, sig.reset)[0] + ";\n"
            else:
                r += "reg " + _printsig_verilog(ns, sig) + ";\n"
    r += "\n"
    return r
...
def _printsync_verilog(f, ns):
    r = ""
    for k, v in sorted(f.sync.items(), key=itemgetter(0)):
        r += "always @(posedge " + ns.get_name(f.clock_domains[k].clk) + ") begin\n"
        r += _printnode_verilog(ns, _AT_SIGNAL, 1, v)
        r += "end\n\n"
    return r


def _printspecials_verilog(overrides, specials, ns, add_data_file):
    r = ""
...
def convert(f, ios=None, name="top",
  ...
  display_run=False, asic_syntax=False):
    ...
    src += _printuse()
    src += _printentitydecl(f, ios, name, ns, reg_initialization=not asic_syntax)
    if False:
